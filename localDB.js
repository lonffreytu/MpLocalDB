/**
 * 小程序本地数据库
 * @tutorial https://github.com/lonffreytu/MpLocalDB
 * @author Lonffrey Tu （新）, JinYufeng
 * @listens MIT
 */
    ...
    if (this.options.orderBy.length) {
        ...
        res.sort((aO, bO) => {
            for (var i = 0, item;
                ...
                const a = aO[item.field]
                const b = bO[item.field]
                ...
                if ((a === null || a === undefined) && !(b === undefined || b === null)) {
                    ...
                    return (isDesc ? -1 : 1)
                } else if (!(a === null || a === undefined) && !(b === undefined || b === null)) {
                    if (typeof a === 'string' && typeof b === 'string') {
                        const comparison = a.localeCompare(b)
                        if (comparison !== 0) {
                            return (isDesc ? -1 : 1) * comparison
                        }
                    } else if (a !== b) {
                        return (isDesc ? 1 : -1) * (a < b ? 1 : -1)
                    }
                }
            }
            return 0
